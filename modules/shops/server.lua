	...
	if data.toType == 'player' then
		...
		if fromData then
			...
			if fromData.license then
				-- first check for a physical license item (weapon_license, driver_license etc)
				local itemName = fromData.license .. '_license'
				local hasItem = Inventory.GetItemCount(playerInv, itemName) > 0
				-- legacy weaponlicense key
				if not hasItem and fromData.license == 'weapon' then
					hasItem = Inventory.GetItemCount(playerInv, 'weaponlicense') > 0
				end
				local hasDb = server.hasLicense and server.hasLicense(playerInv, fromData.license)
				if not hasItem and not hasDb then

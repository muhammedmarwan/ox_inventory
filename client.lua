...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	...
	-- Setup ox_target zones for license purchases
	local licenseZones = {}
	for id, data in pairs(lib.load('data.licenses') or {}) do
		local zoneId = ('license_%s_%d'):format(data.name, id)
			...
			options = {
				{
					name = zoneId,
					icon = 'fa-id-card',
					label = locale('purchase_license', data.name),
					onSelect = function()
						lib.callback('ox_inventory:buyLicense', 1000, function(success, message)
							if success ~= nil then
								lib.notify({
									...
									description = locale(message, locale('license', data.name:gsub("^%l", string.upper)))
								})
							end
						end, id)
					end,
...
-- Vehicle speed restriction based on driver's license
CreateThread(function()
    while true do
        Wait(1000)
        ...
        if IsPedInAnyVehicle(ped, false) then
            ...
            if GetPedInVehicleSeat(vehicle, -1) == ped then
                local hasLicense = lib.callback.await('ox_inventory:hasLicense', false, 'driver')

                if not hasLicense then
                    SetEntityMaxSpeed(vehicle, 5.556)
                else
                    SetEntityMaxSpeed(vehicle, 1000.0) -- removes limit properly
                end
            end
        end
    end
